...
f2_time = time()
f2_df2 = parsweep_multi(larva_fates,f2_p0,f2_pars,[:S,:D,:W],other_args)
f2_time = time() - f2_time

# make figures
f2a = Gadfly.plot(
    filter(row -> (row.μ < 5.) & (row.χm < .5),f2_df2),
    y = :S,
    x = :κm,
    color = :ϵ,
...
f2b = Gadfly.plot(
    filter(row -> (row.μ < 5.) & (1.5 > row.χm > .5),f2_df2),
    y = :S,
    x = :κm,
    color = :ϵ,
    Geom.line,
...
f2c = Gadfly.plot(
    filter(row -> (row.μ < 5.) & (row.χm > 1.5),f2_df2),
    y = :S,
    x = :κm,
    color = :ϵ,
    Geom.line,
